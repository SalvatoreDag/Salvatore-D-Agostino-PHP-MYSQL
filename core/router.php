...
class Router
{
    ...
    public function dispatch()
    {
        ...
        if(!$ret){
            throw new Exception('No routes matched'); //se l'array è vuoto non ha trovato una rottaa
        }
        return $ret;
    }
    ...
    { //metodo per il controllo delle rotte 
        $ret = [];
         foreach($urls as $seg => $classArray){
            if(!str_contains($seg, ':')){ //continua solo se non ho i : nel url (quindi non mi serve l'id)
                continue;
            }


            //posts/:id => posts/5  confronto tra :id e cio' che viene dopo posts/
            //cerca i due punti seguiti da lettere e numeri e lo rimpiazza con la stessa espressione
            //posts/:id viene sostiuita con posts/:[a-zA-Z0-9\-\_] 
            //^ e $ indicano da dove iniza e dove finisce
           ...
           array_shift($matches); //estrae il primo elemento di un array
            $classArray[] = $matches; //abbiamo tolto il primo valore, quindi ora c'è solo l'id
           $ret = $classArray;
           break;
        }
            // var_dump($matches); avro per esempio posts/2 e 2
